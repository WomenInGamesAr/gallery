...
function preload() {
  ...
  var logoSheet = loadSpriteSheet(ASSETS_FOLDER + LOGO_FILE, 60, 90, 2);
    ...
    if (socket.id) {
      ...
      for (var roomId in ROOMS) {
        if (ROOMS.hasOwnProperty(roomId)) {
          ...
          if (room.bg != null) room.bgGraphics = loadImage(ASSETS_FOLDER + room.bg);
          else console.log("WARNING: room " + roomId + " has no background graphics");

          if (room.area != null) room.areaGraphics = loadImage(ASSETS_FOLDER + room.area);
      ...
      dataReady = true;
      print(">>> DATA RECEIVED " + (ROOMS != null));
    }
  });
...
function draw() {
  if (!dataLoaded) {
    dataLoaded = soundsLoaded && roomsLoaded && imagesLoaded && settingsLoaded;
  }
  if (dataLoaded && dataReady) {
    for (var roomId in ROOMS) {
      var room = ROOMS[roomId];

      if (room.bgGraphics != null) if (room.bgGraphics.width == 1) dataLoaded = false;

      if (room.areaGraphics != null) if (room.areaGraphics.width == 1) dataLoaded = false;

      if (room.musicLoop != null) if (!room.musicLoop.isLoaded()) dataLoaded = false;
    }
  ...
  socket.on("playerJoined", function (p) {
    try {
      ...
      if (socket.id == p.id) {
        ...
        if (ROOM_LINK && ROOMS[p.room] != null) window.history.replaceState(null, null, "?room=" + p.room);
        ...
        if (ROOMS[p.room].pageBg != null) document.body.style.backgroundColor = ROOMS[p.room].pageBg;
        ...
        if (ROOMS[p.room].musicLoop != null && SOUND) {
          var vol = 1;
          if (ROOMS[p.room].musicVolume != null) vol = ROOMS[p.room].musicVolume;
...
function update() {
  ...
  //renders the avatar selection screen which can be fully within the canvas
    ...
    fill(0);
    text("Cuerpx", 24 * ASSET_SCALE, 44 * ASSET_SCALE);
    text("Color", 105 * ASSET_SCALE, 44 * ASSET_SCALE);

    text("Elegí tu avatar", 64 * ASSET_SCALE, 18 * ASSET_SCALE);
    ...
    //GUI
    if (nickName != "" && rolledSprite == null && areaLabel == "") animation(walkIcon, floor(mouseX + 6), floor(mouseY - 6));
    ...
    if (longText != "" && nickName != "") {
      ...
      if (longTextLines == -1) {
        ...
        if (longTextAlign == "center" && longTextLines == 1) tw = textWidth(longText + " ");
...
function Player(p) {
  ...
  else if (brightness(c2) > 30) this.sprite.labelColor = BOTTOM_COLORS[p.colors[3]];
...
function canvasPressed() {
  ...
  if (nickName != "" && screen == "game" && mouseButton == RIGHT) {
    if (me.destinationX == me.x && me.destinationY == me.y) socket.emit("emote", { room: me.room, em: true });
  }
}
...
function canvasReleased() {
  ...
  if (screen == "error") {
    ...
    if (me.destinationX == me.x && me.destinationY == me.y) socket.emit("emote", { room: me.room, em: false });
    ...
    case "enter":
      ...
      if (ROOMS[c.room] != null) {
        ...
        if (moment().isBetween(moment(ROOMS[c.room].startDate), moment(ROOMS[c.room].endDate))) {
          ...
          console.log("ERROR: That room is currently closed");
          executeCommand({ cmd: "text", txt: "Esta sala se encuentra cerrada", align: "center", lines: 1 });
        }
      }
      break;
...
function nameValidationCallBack(code) {
  if (socket.id) {
    if (code == 0) {
      ...
      if (e != null) e.innerHTML = "Sorry, only standard western characters are allowed";
    } else {
      hideUser();
      showAvatar();
      avatarSelection();
